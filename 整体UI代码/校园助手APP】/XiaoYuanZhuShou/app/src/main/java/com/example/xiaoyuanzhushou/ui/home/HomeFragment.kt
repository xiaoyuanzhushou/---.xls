...
import com.example.xiaoyuanzhushou.courseTable.course_add.AddCourseActivity
...
//                        Toasty.error(this@HomeFragment, "无法检索背景图片，可能是它为某个应用私有所致，可以尝试在文件管理器中将它移动到其他位置，或是选择其它图片", Toasty.LENGTH_LONG).show()
                        return false
                    }
...
//                    MaterialAlertDialogBuilder(this@HomeFragment)
//                        .setTitle("提示")
...
//                        .show()
            ...
            for (i in 1..7) {
                viewModel.getRawCourseByDay(i, viewModel.table.id).observe(this@HomeFragment, Observer { list ->
...
//    }
    ...
    override fun onBackPressed() {
        when {
            ui.drawerLayout.isDrawerOpen(GravityCompat.START) -> ui.drawerLayout.closeDrawer(
                GravityCompat.START)
            ui.drawerLayout.isDrawerOpen(GravityCompat.END) -> ui.drawerLayout.closeDrawer(
                GravityCompat.END)
